...
def test_an_unchanged_file_reports_no_changes(
    ...
    assert report["modules_changed"] == 0
    assert report["verdict"] == "no VBA changes"
    assert all(entry["status"] == "unchanged" for entry in report["modules"])
...
def test_a_modified_module_is_reported_with_its_diff(
    ...
    assert report["modules_changed"] == 1
    entry = next(e for e in report["modules"] if e["module"] == "Helpers")
...
def test_an_added_module_is_reported_as_added(
    ...
    report = call("xlide_git_changes", file_path=str(committed))
    entry = next(e for e in report["modules"] if e["module"] == "Brand")
...
def test_one_module_can_be_asked_for(call: Callable[..., Any], committed: Path) -> None:
    call(
        "xlide_write_module",
        file_path=str(committed),
        module_name="Helpers",
    ...
    report = call("xlide_git_changes", file_path=str(committed), module_name="helpers")
    assert [e["module"] for e in report["modules"]] == ["Helpers"]
...
def test_a_long_diff_reports_how_much_it_withheld() -> None:
    ...
    from xlide_mcp.tools.vcs import MAX_DIFF_LINES, _diff

    before = "\r\n".join(f"    x = {i}" for i in range(500))
    after = "\r\n".join(f"    y = {i}" for i in range(500))
    result = _diff(before, after, "Big", "modified")
...
def test_a_named_module_that_exists_in_neither_version_is_refused(
    call: Callable[..., Any], committed: Path
) -> None:
    with pytest.raises(ToolFailure) as refusal:
        call("xlide_git_changes", file_path=str(committed), module_name="Ghost")
